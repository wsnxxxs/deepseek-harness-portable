 ...
 */
export function registerReanchorHook(hook: ReanchorHook): () => void {
  hooks.add(hook)
  return () => { hooks.delete(hook) }
}

/** The empty total, so a space with no consumers still reports a receipt. */
export function emptyReanchorOutcome(): ReanchorOutcome {
 ...
 */
export async function runReanchorHooks(
  vault: TopicVault,
  previous: SourceStructure | undefined,
  next: SourceStructure,
): Promise<ReanchorOutcome> {
