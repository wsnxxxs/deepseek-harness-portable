 ...
 */
export function listZipEntries(bytes: Uint8Array): readonly ZipEntry[] {
  ...
  if (entry.method === 8) {
    try {
      ...
      throw new ZipFormatError(
        `data for '${entry.name}' could not be decompressed: ${cause instanceof Error ? cause.message : 'invalid deflate data'}`,
      )
    }
  }
