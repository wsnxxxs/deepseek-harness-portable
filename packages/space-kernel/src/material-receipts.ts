...
interface ReceiptLedger {
  ...
  turn?: number
  active: boolean
  receipts: Map<string, MaterialReceipt>
}

const ledgers = new WeakMap<Agent, ReceiptLedger>()

/** Start the current learner-message evidence window. */
export function beginMaterialTurn(agent: Agent, turn: number): void {
  ledgers.set(agent, {
...
export function materialTurnIsActive(agent: Agent): boolean {
...
export function recordMaterialReceipt(
  agent: Agent,
  ...
  const ledger = existing?.session === agent.session
    ...
    : {
        ...
        turn: undefined,
        active: false,
        receipts: new Map<string, MaterialReceipt>(),
      }
  if (existing === undefined || existing.session !== agent.session) ledgers.set(agent, ledger)
...
export function materialReceiptById(agent: Agent, receiptId: string): MaterialReceipt | undefined {
  const ledger = ledgers.get(agent)
  if (ledger === undefined || ledger.session !== agent.session) return undefined
  return ledger.receipts.get(receiptId)
}

/** Whether a source was structurally mapped in the current evidence window. */
export function materialStructureMapped(agent: Agent, sourceId: string): boolean {
  const ledger = ledgers.get(agent)
  if (ledger === undefined || !ledger.active || ledger.session !== agent.session) return true
  return [...ledger.receipts.values()].some(receipt =>
    receipt.kind === 'structure' && receipt.sourceId === sourceId)
}

/** Return the content receipt for an exact anchor, if one exists. */
export function materialContentReceiptForAnchor(
  agent: Agent,
  anchor: string,
): MaterialReceipt | undefined {
  const ledger = ledgers.get(agent)
  if (ledger === undefined || !ledger.active || ledger.session !== agent.session) return undefined
  return [...ledger.receipts.values()].find(receipt =>
    receipt.kind === 'content' && receipt.anchor === anchor)
}
 ...
 */
export function assertMaterialAnchorsReadable(agent: Agent, anchors: readonly string[]): void {
  ...
  if (missing.length > 0) {
    throw new TypeError(
      `source_anchors_observed requires a material read receipt for: ${missing.join(', ')}. `
