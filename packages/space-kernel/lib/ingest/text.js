...
export const MARKDOWN_EXTENSIONS = ['md', 'markdown', 'mdx', 'mdown'];
/** Extensions the plain-text parser owns. */
export const PLAIN_TEXT_EXTENSIONS = ['txt', 'text', 'rst', 'org', 'log', 'csv', 'tsv'];
...
class BlockBuilder {
    ...
    heading(text, level, page) {
        ...
        while (this.headingPath.length < depth - 1)
            this.headingPath.push('');
 ...
 */
export function parseMarkdownBlocks(text, sourceId, title) {
    ...
    for (; index < lines.length; index += 1) {
        ...
        if (fence !== undefined && fence !== null) {
            ...
            for (; index < lines.length; index += 1) {
                ...
                if (inner.trimStart().startsWith(marker.slice(0, 3)))
                    break;
                body.push(inner);
            }
 ...
 * @returns blocks in document order.
 */
export function parsePlainTextBlocks(text, sourceId, title) {
    const builder = new BlockBuilder(sourceId);
    builder.heading(title, 1);
    const lines = text.split(/\r?\n/);
    let paragraph = [];
 ...
 * @returns blocks in document order.
 */
export function parseCodeBlocks(text, sourceId, title, lang) {
    const builder = new BlockBuilder(sourceId);
    builder.heading(title, 1);
    const lines = text.split(/\r?\n/);
    let segment = [];
 ...
 */
export function parseTextSource(text, options) {
