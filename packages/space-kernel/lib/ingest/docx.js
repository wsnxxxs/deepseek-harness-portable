 ...
 */
import { readZipText, ZipFormatError } from "./zip.js";
...
function elementEnd(xml, start, tag) {
    ...
    if (selfClosing !== null)
        return start + selfClosing[0].length;
...
function headingLevel(xml) {
    ...
    if (named !== null)
        return Math.min(Number.parseInt(named[1] ?? '1', 10) + 1, 6);
 ...
 */
export function parseDocxSource(bytes, options) {
    ...
    for (;;) {
        ...
        while (headingPath.length < depth - 1)
            headingPath.push('');
