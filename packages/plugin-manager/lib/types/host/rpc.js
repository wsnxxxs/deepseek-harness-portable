...
import { composeRows, nextPreferences, portablePreferences, toggleOutcome, } from "./registry.js";
 ...
 * @returns the lookup.
 */
export function packageFactsFrom(anchor) {
...
export function defaultDeps(loader, anchor) {
    return {
        loader,
...
export function listPortablePlugins(deps) {
    try {
 ...
 */
export function setPortablePluginEnabled(deps, payload) {
    ...
    if (row === undefined)
        return failure('not-builtin', `${name} is not a built-in feature of this build`, { name });
    try {
        let preferences = nextPreferences(portablePreferences(manifest), name, enabled);
        if (enabled) {
            for (const dependency of REQUIREMENTS[name] ?? [])
                preferences = nextPreferences(preferences, dependency, true);
        }
        else {
            for (const [dependent, dependencies] of Object.entries(REQUIREMENTS)) {
                if (dependencies.includes(name))
                    preferences = nextPreferences(preferences, dependent, false);
            }
        }
        const bundles = new Set(manifest.dsh?.profile?.bundles ?? []);
        for (const bundle of FEATURE_BUNDLES) {
            if (preferences[bundle] === true)
                bundles.add(bundle);
            if (preferences[bundle] === false)
 ...
 */
export function handlePortablePluginEndpoint(endpoint, payload, deps) {
