...
import { dossierMap, dossierRead, dossierSearch, openDossier } from "./dossier.js";
/** Refuse extra properties, matching every other tool in this distribution. */
function closed(tool) {
 ...
 */
export function registerDossierTools(ctx) {
    const disposers = [];
    ...
    disposers.push(ctx.tools.register(closed(defineTool({
        ...
        async execute(args, exec) {
            if (args.query === undefined)
                return { ok: false, reason: 'dossier_search needs a query' };
            const vault = await vaultOf(exec.agent);
            if (vault === undefined)
                return NO_DOSSIER;
