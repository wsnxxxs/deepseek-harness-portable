...
// Type-only imports pull the declaration merges (ctx.sessions, ctx.locale,
// ctx.inputTriggers, ctx.remote, ctx.slots, the SlotMap keys, and the session
// standard props that carry `useInput`) into this compilation unit without
// adding a runtime module edge.
...
import { ComposerAttachButton, type ComposerAttachInjected } from './AttachButton.tsx'
import { filesCommand, type PickedReference } from './files-command.ts'
import { SpanRegistry } from './spans.ts'
import { COMPOSER_ATTACH_NS, en, zh, type ComposerAttachKey } from './locales.ts'
...
export {
  ...
  type FilesCommandDeps, type PickedReference,
} from './files-command.ts'
export { SpanRegistry } from './spans.ts'
export {
  COMPOSER_ATTACH_NS, en, zh,
  type ComposerAttachKey, type ComposerAttachTranslate,
} from './locales.ts'
...
export function apply(ctx: ClientContext): void {
  ...
  const insert = (session: ClientSessionContext, reference: PickedReference): boolean => {
    const span = spans.read(session.sessionId)
    const actx = ctx.sessions.scope(session.sessionId)
    if (span === undefined || actx === undefined) return false
  ...
  ctx.effect(() => (ctx.get('commandUi') as CommandUiContract).register(filesCommand({
  ...
  const attachOperations = (sessionId: SessionId): ComposerAttachInjected => ({
