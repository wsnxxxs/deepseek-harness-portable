...
const BUILD_INPUT_PATHS = [
  ...
  'packages/desktop-enhancements',
  'packages/connection-rpc',
  'packages/web-plugins',
  'packages/plugin-manager',
  'apps/runtime/cordis.patch.yml',
  'apps/dcode-ui/cordis.patch.yml',
  'apps/cluster-ui/cordis.patch.yml',
  'apps/interactive-learning/cordis.patch.yml',
  'scripts/runtime',
...
class DesktopExeBuild {
  ...
  async deployStaging(): Promise<void> {
    ...
    await this.preserveHostInstallState(() => this.run('deploy', pnpmBin(), [
      ...
      '--legacy',
      '--prod',
      // Upstream's pkg patch targets a build-only dependency absent from this closure.
