...
- Preinstalls the pinned `@linxin666/dsh-web-all@0.3.16` once per Web profile; users can disable or remove it without the distribution restoring it on restart.
...
For a disposable Windows x64 1.6.2 test build, use a separate output root:
...
`dist-desktop/electron-v1.6.2-test/windows-artifacts/`.
...
- Release: DeepSeek Harness Desktop v1.6.2
