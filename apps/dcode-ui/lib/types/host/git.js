...
export async function readStatus(cwd) {
    const empty = { repository: false, detached: false, ahead: 0, behind: 0, files: [], truncated: false, insertions: 0, deletions: 0 };
    let root;
    try {
 ...
 */
export async function readDiff(cwd, path, staged) {
 ...
 */
export async function readBranches(cwd) {
    const root = await workTreeRoot(cwd);
    if (root === undefined)
        return [];
 ...
 */
export async function undoPaths(cwd, paths) {
    const root = await workTreeRoot(cwd);
    if (root === undefined)
        throw new Error('not a git work tree');
