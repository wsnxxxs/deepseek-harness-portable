...
export function PlanCard({ sessionId, open = true, navigation }) {
    ...
    return (_jsx("div", { className: css.dock, children: _jsxs("section", { className: css.card, "data-testid": "dcode-plan-card", "aria-label": todos.length > 0 ? t('plan.title') : t('trace.title'), children: [_jsxs("button", { type: "button", className: `${css.header} ${ui.cardHeader}`, "aria-expanded": !collapsed, "aria-controls": contentId, onClick: () => { setCollapsed(value => !value); }, children: [_jsx("span", { className: css.icon, "aria-hidden": true, children: _jsx(IconChecklistOutline14, { size: 16 }) }), _jsx("span", { className: css.title, children: todos.length > 0 ? t('plan.title') : t('trace.title') }), _jsx("span", { className: css.progress, children: todos.length > 0
                                ? t('plan.progress', { done: completed, total: todos.length })
                                : t('trace.stats', { events: trajectory?.eventNodes.length ?? 0, requests: trajectory?.requests.length ?? 0 }) }), _jsx("span", { className: css.chevron, "aria-hidden": true, children: collapsed ? _jsx(IconChevronRightOutline14, {}) : _jsx(IconChevronDownOutline14, {}) })] }), !collapsed ? _jsxs("div", { id: contentId, children: [todos.length > 0
