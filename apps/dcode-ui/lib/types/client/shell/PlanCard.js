...
function StatusMark({ status }) {
    ...
    if (status === 'in_progress') {
        return _jsx("span", { className: `${css.mark} ${css.markActive}`, "aria-hidden": true });
    }
...
export function PlanCard({ sessionId, open = true }) {
    ...
    return (_jsx("div", { className: css.dock, children: _jsxs("section", { className: css.card, "data-dcode-plan-card": "", "data-testid": "dcode-plan-card", "aria-label": t('plan.title'), children: [_jsxs("button", { type: "button", className: `${css.header} ${ui.cardHeader}`, "aria-expanded": !collapsed, "aria-controls": contentId, onClick: () => { setCollapsed(value => !value); }, children: [_jsx("span", { className: css.icon, "aria-hidden": true, children: _jsx(IconChecklistOutline14, { size: 16 }) }), _jsx("span", { className: css.title, children: t('plan.title') }), _jsx("span", { className: css.progress, children: t('plan.progress', { done: completed, total: todos.length }) }), _jsx("span", { className: css.chevron, "aria-hidden": true, children: collapsed ? _jsx(IconChevronRightOutline14, {}) : _jsx(IconChevronDownOutline14, {}) })] }), !collapsed
                    ? (_jsx("div", { id: contentId, className: css.body, children: _jsx("ul", { className: css.list, children: todos.map((todo, index) => (_jsxs("li", { className: css.item, "data-status": todo.status, children: [_jsx(StatusMark, { status: todo.status }), _jsx("span", { className: css.content, children: todo.content })] }, `${String(index)}:${todo.content}`))) }) }))
                    : null] }) }));
}
