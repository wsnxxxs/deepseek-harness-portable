    ...
    return (_jsxs("nav", { className: css.rail, "data-dsh-surface": "sidebar", "aria-label": t('app.title'), children: [_jsxs("div", { className: css.top, children: [_jsxs("div", { className: css.brand, children: [_jsx("span", { className: css.brandMark, "aria-hidden": true, children: _jsx(IconSparkle16, {}) }), _jsxs("span", { className: css.brandCopy, children: [_jsx("strong", { children: "DCode" }), _jsx("span", { children: t('nav.agentWorkspace') })] })] }), _jsxs("label", { className: css.searchField, children: [_jsx(IconSearchOutline16, {}), _jsx("input", { ref: searchRef, type: "search", className: css.searchInput, value: query, placeholder: t('common.search'), "aria-label": t('common.search'), onChange: event => { setQuery(event.target.value); }, onKeyDown: (event) => {
                    ...
                    moveSession(event.key === 'ArrowDown' ? 1 : -1);
                        ...
                        ? (_jsxs(_Fragment, { children: [filteredGroups.map(group => (_jsxs("div", { className: css.group, children: [_jsx(WorkspaceRow, { group: group, collapsed: collapsed.has(group.workspaceId), onToggle: () => { toggleGroup(group.workspaceId); }, onNewTask: () => { onNewTask(group.workspaceId); }, onRename: () => { openRename(group); }, onRemove: () => { openRemove(group); } }), collapsed.has(group.workspaceId)
                                            ? null
                                            : group.sessions.map(session => (_jsx(SessionRow, { session: session, current: session.id === list.current, age: age(session.updatedAt), onOpen: () => { openSession(session); }, onRename: () => { openSessionRename(session); }, onArchive: () => { void runtime.workspaces.archiveSession(session.id); } }, session.id)))] }, group.workspaceId))), filteredUngrouped.length === 0
                                    ? null
                                    : (_jsxs("div", { className: css.group, children: [_jsx("div", { className: css.groupHeader, children: _jsx("span", { className: css.groupName, children: t('nav.ungrouped') }) }), filteredUngrouped.map(session => (_jsx(SessionRow, { session: session, current: session.id === list.current, age: age(session.updatedAt), onOpen: () => { openSession(session); }, onRename: () => { openSessionRename(session); }, onArchive: () => { void runtime.workspaces.archiveSession(session.id); } }, session.id)))] }))] }))
