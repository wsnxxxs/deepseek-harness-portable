...
function PluginConfigSection(props) {
    ...
    return (_jsxs("div", { className: css.pluginConfigList, children: [props.data.credentialError === undefined ? null : _jsx("div", { className: css.notice, children: `${t('settings.plugins.credentialWarning')}: ${props.data.credentialError}` }), cards.length === 0 ? _jsx(EmptyState, { children: t('settings.plugins.emptyConfig') }) : cards] }));
}
function PluginInventory(props) {
    ...
    return (_jsxs("div", { className: css.pluginInventory, children: [_jsx("input", { className: css.search, type: "search", value: query, placeholder: t('settings.plugins.search'), onChange: event => { setQuery(event.target.value); } }), _jsxs("div", { className: css.inventoryHeading, children: [_jsx("span", { className: css.sectionTitle, children: t('settings.plugins.inventoryTitle') }), _jsx("span", { className: css.badge, children: filtered.length })] }), filtered.length === 0 ? _jsx(EmptyState, { children: t('settings.plugins.emptyInventory') }) : (_jsx("div", { className: css.card, children: filtered.map(entry => {
                    const open = expanded === entry.entryId;
                    return (_jsxs("div", { className: css.inventoryRow, children: [_jsxs("button", { type: "button", className: css.inventoryButton, "aria-expanded": open, onClick: () => { setExpanded(current => current === entry.entryId ? undefined : entry.entryId); }, children: [_jsxs("span", { className: css.rowText, children: [_jsx("span", { className: css.rowTitle, children: entry.moduleName }), _jsx("span", { className: css.rowBody, children: entry.enabled ? entry.fiberPhase ?? t('settings.plugins.unobserved') : t('settings.plugins.disabled') })] }), _jsx("span", { className: css.badge, children: entry.enabled ? t('settings.plugins.enabled') : t('settings.plugins.disabled') })] }), open ? _jsx("code", { className: css.inventoryDetails, children: entry.entryId }) : null] }, entry.entryId));
                }) }))] }));
}
...
export function PluginSettingsSection({ mcpOnly = false }) {
    ...
    return (_jsxs("section", { className: css.section, children: [_jsx("span", { className: css.sectionTitle, children: mcpOnly ? t('settings.mcp') : t('plugins.section.settings') }), _jsx("p", { className: css.sectionBody, children: mcpOnly ? t('settings.plugins.mcpBody') : t('settings.pluginsBody') }), !mcpOnly ? (_jsxs("div", { className: css.pluginTabs, role: "tablist", "aria-label": t('settings.plugins.tabs'), children: [_jsx("button", { type: "button", role: "tab", "aria-selected": tab === 'config', className: `${css.pluginTab} ${tab === 'config' ? css.pluginTabActive : ''}`, onClick: () => { setTab('config'); }, children: t('settings.plugins.configTab') }), _jsx("button", { type: "button", role: "tab", "aria-selected": tab === 'inventory', className: `${css.pluginTab} ${tab === 'inventory' ? css.pluginTabActive : ''}`, onClick: () => { setTab('inventory'); }, children: t('settings.plugins.inventoryTab') })] })) : null, !mcpOnly && tab === 'config' ? _jsx(PluginConfigSection, { data: value, onReload: data.reload }) : _jsx(PluginInventory, { data: value, mcpOnly: mcpOnly })] }));
}
