 ...
 */
export function useOperations(client) {
    ...
    const put = useCallback((key, operation) => {
        if (!live.current)
            return;
    ...
    const poll = useCallback((key, jobId, onSuccess) => {
        stopTimer(key);
        let failures = 0;
        const timer = setInterval(() => {
            void client.job(jobId).then((answer) => {
                ...
                if (!answer.ok) {
                    ...
                    failures += 1;
                    if (!answer.unavailable && failures < POLL_FAILURE_LIMIT)
                        return;
                    stopTimer(key);
    ...
    const start = useCallback((key, call, onSuccess) => {
...
//# sourceMappingURL=useJob.js.map