...
const UNMEASURED: LayoutSize = 'medium'
 ...
 */
export function resolveLayoutSize(width: number): LayoutSize {
 ...
 */
export function fitPanels(size: LayoutSize, current: PanelState): PanelState {
  if (size === 'compact') {
 ...
 */
export function initialLayoutSize(): LayoutSize {
 ...
 */
function inlineSizeOf(entry: ResizeObserverEntry): number {
 ...
 */
export function useLayoutSize(node: HTMLElement | null): LayoutSize {
  const [size, setSize] = useState<LayoutSize>(initialLayoutSize)
  useEffect(() => {
    if (node === null) return undefined
    if (typeof ResizeObserver === 'undefined') {
      // No observer: the window is the only width signal available, and on
      // this shell the frame spans it.
      const onResize = (): void => { setSize(resolveLayoutSize(node.clientWidth || window.innerWidth)) }
      onResize()
    ...
    const observer = new ResizeObserver((entries) => {
      ...
      if (entry !== undefined) setSize(resolveLayoutSize(inlineSizeOf(entry)))
    })
