...
export function xtermColor(index: number): string {
  ...
  if (index < 232) {
    ...
    return rgbColor(
      CUBE_LEVELS[Math.floor(offset / 36)] ?? 0,
      CUBE_LEVELS[Math.floor(offset / 6) % 6] ?? 0,
      CUBE_LEVELS[offset % 6] ?? 0,
    )
  }
 ...
 */
function nextValue(params: readonly string[], from: number): number {
  let index = from
 ...
 */
export function applySgr(style: Style, params: readonly string[]): Style {
  ...
  for (let index = 0; index < params.length; index += 1) {
    ...
    if (code >= 30 && code <= 37) { next.fg = paletteColor(code - 30); continue }
    if (code === 39) { delete next.fg; continue }
    if (code >= 40 && code <= 47) { next.bg = paletteColor(code - 40); continue }
    if (code === 49) { delete next.bg; continue }
    if (code >= 90 && code <= 97) { next.fg = paletteColor(code - 90 + 8); continue }
    if (code >= 100 && code <= 107) { next.bg = paletteColor(code - 100 + 8); continue }
 ...
 */
function resolveInverse(style: Style): Style {
 ...
 */
export function parseAnsi(text: string, limits: AnsiLimits = {}): AnsiDocument {
 ...
 */
function skipEscape(text: string, start: number, onSgr: (params: readonly string[]) => void): number {
  ...
  if (kind === '[') {
    ...
    while (cursor < text.length) {
      ...
      if (char >= '0' && char <= '?') { cursor += 1; continue }
      break
    }
    const paramsEnd = cursor
    while (cursor < text.length) {
      const char = text[cursor] ?? ''
      if (char >= ' ' && char <= '/') { cursor += 1; continue }
      break
    }
    const final = text[cursor]
    if (final === undefined) return text.length
    if (final === 'm') {
  ...
  if (kind === ']' || kind === 'P' || kind === 'X' || kind === '^' || kind === '_') {
    ...
    while (cursor < text.length) {
      ...
      if (char === '\u001b' && text[cursor + 1] === '\\') return cursor + 2
      cursor += 1
    }
    return text.length
  }
