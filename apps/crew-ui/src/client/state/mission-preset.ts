...
function presetOf(summary: MissionSummary | undefined): string | undefined {
 ...
 */
export function stageAction(summary: MissionSummary | undefined): StageAction {
  if (summary === undefined) return 'wait'
 ...
 */
export function useMissionPreset(
  ...
  const start = useCallback((workspaceId?: string) => {
  ...
  useEffect(() => {
    if (!staged.current || current === undefined) return
