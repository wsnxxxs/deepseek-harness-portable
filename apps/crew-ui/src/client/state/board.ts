...
import type {
  CreateTeamTaskRequest, TeamTaskId, TeamTaskMutationResult, TeamTaskView, TeamView,
...
export function groupTasks(tasks: readonly TeamTaskView[]): Record<BoardColumn, TeamTaskView[]> {
  const grouped: Record<BoardColumn, TeamTaskView[]> = { pending: [], in_progress: [], completed: [] }
 ...
 */
export function blockerLabels(task: TeamTaskView, tasks: readonly TeamTaskView[]): string[] {
 ...
 */
export function leadSessionId(sessions: ISessions, sessionId: SessionId): SessionId {
 ...
 */
export function useBoard(sessionId: SessionId | undefined): BoardState {
  const runtime = useRuntime()
  const [view, setView] = useState<TeamView | undefined>(undefined)
  const [loading, setLoading] = useState(false)
  const [error, setError] = useState<string | undefined>(undefined)
  const [pending, setPending] = useState<ReadonlySet<string>>(() => new Set())

  const lead = useMemo(
    () => (sessionId === undefined ? undefined : leadSessionId(runtime.sessions, sessionId)),
    [runtime, sessionId],
  )
  ...
  const read = useCallback(async (first: boolean): Promise<void> => {
    const target = leadRef.current
    if (target === undefined) return
    const mine = ++generation.current
    if (first) setLoading(true)
    let result: RemoteResult<TeamView>
    try {
  ...
  const settle = useCallback(async (
    key: string,
    operation: (target: SessionId) => Promise<RemoteResult<TeamTaskMutationResult>>,
    ...
    try {
      let result: RemoteResult<TeamTaskMutationResult>
      try {
        ...
        if (leadRef.current !== target) return undefined
        setError(transportText(error))
        return undefined
      }
      if (leadRef.current !== target) return undefined
  ...
  const create = useCallback(
    async (request: CreateTeamTaskRequest) => settle('create', target => runtime.teams.createTask(target, request)),
    [runtime, settle],
  )

  const update = useCallback(
    async (request: UpdateTeamTaskRequest) => settle(String(request.taskId), target => runtime.teams.updateTask(target, request)),
    [runtime, settle],
  )
