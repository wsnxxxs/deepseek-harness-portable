...
```
uiMode = 'zcode'     → register Workbench into root   → AppFrame is shadowed
...
| URL parameter | `?view=zcode` / `?view=official` |

Boot resolution order: URL parameter → `localStorage` → the desktop config's
`uiMode` → `zcode`. The active mode is written back to the address bar with
...
Version control. The host half of this package serves a `/zcode` Connection
...
HEAD; an untracked file is *moved* into `.dsh/zcode-undo/<timestamp>/`, never
...
src/
  ...
  index.ts              host plugin: the /zcode channel
...
```bash
pnpm --filter @dsh-portable/zcode-ui run build
```
...
- The `/zcode` channel takes new endpoints by extending `ZCODE_ENDPOINTS` and
