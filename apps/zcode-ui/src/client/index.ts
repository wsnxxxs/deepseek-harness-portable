 ...
 * well.
 * @module @dsh-portable/zcode-ui/client
 */
...
import { createZcodeRuntime, ZcodeRuntimeProvider } from './state/runtime.ts'
import { bindTranslate, TranslateProvider } from './state/i18n.ts'
import { ZCODE_NS, en, zh, type ZcodeKey } from './locales.ts'
...
export { createZcodeRuntime, type ZcodeRuntime } from './state/runtime.ts'
...
export { ZCODE_NS, type ZcodeKey } from './locales.ts'
...
export const name = 'zcode-ui-client'
...
function bindRootRegistration(ctx: ClientContext, mode: UiModeStore): () => void {
  ...
  const runtime = createZcodeRuntime(ctx, mode)
  const t = bindTranslate(ctx.locale.bind(ZCODE_NS))
  ...
  const apply = (): void => {
    ...
    active = ctx.slots.inject('root', () => ctx.slots.register(
      { name: 'root', priority: ROOT_PRIORITY, locale: ZCODE_NS },
      render,
    ))
  }
...
export function apply(ctx: ClientContext): void {
  ctx.effect(() => ctx.locale.register(ZCODE_NS, { zh, en }), 'zcode-ui: dictionaries')

  const mode = createUiModeStore()
  ctx.effect(() => () => { mode.dispose() }, 'zcode-ui: mode store')
  ctx.effect(() => bindRootRegistration(ctx, mode), 'zcode-ui: root surface')
