...
const MODE_SUPPORT_RANK = { native: 0, compatible: 1, alternative: 2 };
/** Selectable modes, best-supported first, ties broken by id. */
export function selectableModes(catalog) {
    return Object.values(catalog.modes)
        .filter(mode => mode.selectable)
        .sort((left, right) => ((MODE_SUPPORT_RANK[left.supportLevel] ?? Number.MAX_SAFE_INTEGER)
 ...
 */
export function reconcilePresetRoster(overlays, catalog) {
    const overlay = overlays.find(entry => entry.id === 'agent-presets');
    ...
    if (typeof configured !== 'string')
        return { kind: 'intact' };
    if (catalog.modes[canonicalModeId(configured)]?.selectable === true)
        return { kind: 'intact' };
