...
export function ensureWebAllProfile(profileDir: string, bundledSourceDir?: string): void {
  const path = join(profileDir, 'package.json')
  const manifest = JSON.parse(readFileSync(path, 'utf8'))
  manifest.dsh ??= {}
  manifest.dsh.profile ??= {}
  const profile = manifest.dsh.profile
  if (profile.portableWebAllSeeded === true) return
  const bundles: string[] = profile.bundles ?? []
  profile.bundles = bundles.filter(name => name !== 'dsh-plugin-marketplace')
  if (!profile.bundles.includes(WEB_ALL_PACKAGE)) profile.bundles.push(WEB_ALL_PACKAGE)
