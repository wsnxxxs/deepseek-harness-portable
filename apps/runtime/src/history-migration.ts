...
function normalizeLegacyRow(row: Row): Row {
  const result = structuredClone(row)
  if (result.type === 'subagent/descriptor' && result.data.version === 2) {
    result.data.version = 3
  }
  if (result.type === 'permission/preset') delete result.data.origin
...
export function decodeHistory(bytes: Buffer, compressed: boolean): Row[] {
  const chunks: Buffer[] = []
  if (!compressed) chunks.push(bytes)
  else {
    let offset = 0
    while (offset < bytes.length) {
      ...
      if (result.engine.bytesWritten <= 0) throw new Error('Historical Zstandard frame made no progress')
...
export function migrateHistory(rows: Row[]): Buffer | undefined {
  const header = rows[0]
  if (!header || header.version >= sessionFormatCatalog.currentVersion) return
  ...
  const events = artifact.events.map(event => {
    const original = event.type === 'feedback/record' ? originals.get((event.data as Row).text) : undefined
    if (!original) return event
    restored += 1
    return { ...event, type: original.type, data: original.data, ignorable: true } as SessionFormatEvent
  })
  if (restored !== owned.length) throw new Error('Historical extension metadata was not preserved')
  const physical = [
  ...
  for (const row of physical.slice(1)) verify.decodeRow(row)
  verify.finish()
...
export async function migrateHistoryFile(source: string): Promise<boolean> {
  ...
  if (names.some(name => Number(/^session\.v(\d+)\.jsonl(?:\.zstd)?$/.exec(name)?.[1] ?? 0) > version)) return false
  ...
  try {
    ...
    if (before.size !== after.size || before.mtimeMs !== after.mtimeMs) throw new Error('Historical session changed during migration')
...
export async function migratePortableHistories(root: string): Promise<{ migrated: number; failures: string[] }> {
  const result = { migrated: 0, failures: [] as string[] }
  async function visit(directory: string): Promise<void> {
    ...
    for (const entry of candidates.sort((a, b) => b.name.localeCompare(a.name))) {
      try { if (await migrateHistoryFile(join(directory, entry.name))) result.migrated += 1 }
      catch (error) { result.failures.push(`${directory}: ${(error as Error).message}`) }
    }
