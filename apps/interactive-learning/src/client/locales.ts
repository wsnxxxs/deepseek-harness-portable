export const zh = {
  ...
  vaultGroupNotes: '笔记',
  vaultLocalOnly: '这一页只读取本地文件，不会调用模型。',
  ...
  vaultConceptsEmptyBody: '概念卡不是随手保存出来的：要等你自己在一个新语境里独立把它用对一次，系统才会提议存一张。所以这里稀疏是正常的。',
  ...
  vaultCancel: '取消',
  vaultSaving: '正在保存…',
  vaultSystemFieldsNote: '掌握度、到期日和引用由系统维护，这里不提供直接编辑。在 Obsidian 里手改到期日会被尊重，手改掌握度会在下次同步时被覆盖回去。',
  vaultCorrect: '我当时其实没懂',
  vaultCorrectHint: '把掌握度降一级并立刻排进复习。只能往下——想升回去要在对话里再做对一次。',
  vaultCorrectDone: '已降为{mastery}，并排进今天的复习',
  vaultDefer: '推迟',
  vaultDeferDays: '推迟天数',
  vaultDeferHint: '只改到期日，不动间隔——推迟一次不会污染后续排期。',
  vaultOpenFile: '查看文件',
  vaultCloseFile: '收起文件',
  vaultReviewEmptyTitle: '今天没有到期的卡片',
  vaultReviewEmptyBody: '到期的卡片会自动出现在这里。复习全程在本地完成，不消耗 token。',
  ...
  vaultNotesPendingHint: '这些是你想留成概念卡、但还没满足条件的草稿。概念卡要等你在对话里，于一个新语境下独立把这个概念用对一次，才会由系统生成。在那之前它们就先安静待在这里，不进复习队列，也不进模型的学习地图。',
  ...
  vaultNoteGateUnmet: '本轮未满足',
  vaultNotePromote: '并入概念卡',
  vaultNotePromoteHint: '并入只是把这段文字追加到已存在的概念卡上，不会新建卡、不会改掌握度。想让一个概念第一次成卡，只有一条路：在对话里独立做对一次迁移。',
  ...
  vaultKeepAsCardUnmet: '存为概念卡（本轮未满足）',
  vaultKeepAsCardHint: '概念卡不能从一条讲解直接生成。它记录的是你自己的掌握程度，所以要等你在一个新语境里独立把它用对一次，系统才会写卡。现在可以先存成待确认草稿。',
  ...
  vaultReparseOffer: '这些页能重新读吗？',
  vaultReparseRun: '让 {model} 读这 {count} 页',
  vaultReparseRunning: '正在读 {count} 页…',
  vaultReparseExplain: '第 {pages} 页会被渲染成图片交给 {model} 转写。这是这个面板里唯一会发起模型生成、消耗 token 的操作；查看路线时还会查询 provider/model catalog，但不会生成内容。转写出来的文字会标成「视觉重读」——它是模型看图写的，不是原文的文字层。',
  vaultReparseUnknownCapability: '{model} 没有声明是否接受图像输入。这次调用可能会被服务端拒绝，但仍然会计入请求。',
  vaultReparseTextOnly: '{model} 明确不接受图像输入，所以这里不会替你偷偷换一个模型去读。换成一个多模态模型后再回来。',
  ...
  vaultRosterLocal: '这些数字来自本地文件，打开面板不会调用模型。',
}
...
export const en: typeof zh = {
  ...
  vaultLocalOnly: 'This page reads local files only. It never calls a model.',
  ...
  vaultConceptsEmptyBody: 'A card is not something you save on a whim: one is proposed only after you have used the idea correctly, on your own, in a context you have not seen before. Sparse here is the intended state.',
  ...
  vaultCancel: 'Cancel',
  vaultSaving: 'Saving…',
  vaultSystemFieldsNote: 'Mastery, due date and citations are maintained by the system and are not editable here. Editing the due date by hand in Obsidian is honoured; editing mastery by hand is overwritten on the next sync.',
  vaultCorrect: "I didn't actually understand this",
  vaultCorrectHint: 'Drops mastery one step and schedules it for today. Downward only — raising it means getting it right again in a conversation.',
  vaultCorrectDone: 'Lowered to {mastery} and scheduled for today',
  vaultDefer: 'Defer',
  vaultDeferDays: 'Days to defer',
  vaultDeferHint: 'Moves the due date only, never the interval — deferring once does not distort later spacing.',
  vaultOpenFile: 'View file',
  vaultCloseFile: 'Hide file',
  vaultReviewEmptyTitle: 'Nothing due today',
  vaultReviewEmptyBody: 'Cards appear here when they come due. Reviewing happens entirely on this machine and spends no tokens.',
  ...
  vaultNotesPendingHint: 'Drafts you want to become concept cards but that do not qualify yet. A card is written only after you use the idea correctly, on your own, in a context you have not seen before. Until then these sit here quietly: out of the review queue, and out of the model’s study map.',
  ...
  vaultNoteGateUnmet: 'Not met this round',
  vaultNotePromote: 'Merge into card',
  vaultNotePromoteHint: 'Merging only appends this text to a card that already exists. It never creates a card and never changes mastery. There is exactly one way a concept gets its first card: transfer it correctly, on your own, in conversation.',
  ...
  vaultKeepAsCardUnmet: 'Keep as concept card (not met this round)',
  vaultKeepAsCardHint: 'A concept card cannot be made from an explanation. It records what YOU have mastered, so one is written only after you use the idea correctly and unaided in a context you have not seen. For now, keep it as a pending draft.',
  ...
  vaultReparseOffer: 'Can these pages be re-read?',
  vaultReparseRun: 'Have {model} read these {count} page(s)',
  vaultReparseRunning: 'Reading {count} page(s)…',
  vaultReparseExplain: 'Page(s) {pages} get rendered to images and transcribed by {model}. This is the only action in this panel that starts model generation and spends tokens; checking the route also queries the provider/model catalog, but does not generate content. Recovered text is marked "视觉重读" — a model read it off a picture, it is not the file’s own text layer.',
  ...
  vaultReparseTextOnly: '{model} declares that it does not accept images, and nothing here will quietly swap in another model to read them for you. Switch to a multimodal model and come back.',
  ...
  vaultRosterLocal: 'These counts come from local files; opening this calls no model.',
}
