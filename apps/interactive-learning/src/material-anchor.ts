 ...
 */
export function formatSectionAnchor(sourceId: string, section: SourceSection): string {
...
export function anchorTargetsOf(structure: SourceStructure): readonly AnchorTarget[] {
...
export function formatAnchorTarget(target: AnchorTarget): string {
 ...
 */
export function resolveAnchorTarget<T extends AnchorTarget>(
  anchor: string,
  targets: readonly T[],
): T | undefined {
  ...
  for (const target of targets) {
    if (sourceId !== undefined && normalizeQuote(target.sourceId).toLowerCase() !== sourceId) continue
    if (parsed.page !== undefined && target.page !== parsed.page) continue
    const candidates = [
      target.sectionId,
...
export function anchorPage(anchor: string): number | undefined {
 ...
 */
export function parseAnchorText(anchor: string): ParsedAnchor {
...
export function sectionMentions(text: string): readonly string[] {
  const found: string[] = []
...
export function mentionSupported(mention: string, targets: readonly AnchorTarget[]): boolean {
