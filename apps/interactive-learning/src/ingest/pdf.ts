...
/** The narrow slice of pdf.js this parser depends on. */
interface TextItem {
  str?: string
  height?: number
  width?: number
...
interface DocumentProxy {
  numPages: number
  getPage(pageNumber: number): Promise<PageProxy>
}

interface UnpdfModule {
  getDocumentProxy(data: Uint8Array): Promise<DocumentProxy>
}
...
let unpdfModule: Promise<UnpdfModule | undefined> | undefined

/** Load `unpdf` once, or report that this build cannot read pdf. */
async function loadUnpdf(): Promise<UnpdfModule | undefined> {
  unpdfModule ??= import(UNPDF_SPECIFIER)
    .then(module => module as UnpdfModule)
    .catch(() => undefined)
  return await unpdfModule
}

/** One recovered visual line of a page. */
interface PdfLine {
  text: string
  size: number
  /** Left edge, used only for the multi-column check. */
  x: number
}
...
function linesOf(items: readonly TextItem[]): readonly PdfLine[] {
  const rows: { y: number; x: number; size: number; parts: string[] }[] = []
...
function numberedDepth(text: string): number | undefined {
  const dotted = /^(\d{1,2}(?:\.\d{1,2}){0,3})\s+\S/.exec(text)
  if (dotted === null) return undefined
 ...
 */
function looksMultiColumn(lines: readonly PdfLine[], width: number): boolean {
...
export async function parsePdfSource(
  ...
  const pages: { lines: readonly PdfLine[]; width: number; mathRuns: number }[] = []
  const imageOnly: number[] = []
  const multiColumn: number[] = []
  let truncated: ParseDegradation | undefined
  ...
  for (let pageNumber = 1; pageNumber <= document.numPages; pageNumber += 1) {
    try {
      ...
      if (chars < MIN_PAGE_CHARS) imageOnly.push(pageNumber)
  ...
  if (imageOnly.length > 0) degradation.push({ kind: 'image-only-pages', pages: imageOnly })
  if (multiColumn.length > 0) degradation.push({ kind: 'multi-column-guess', pages: multiColumn })
  const mathRuns = pages.reduce((total, page) => total + page.mathRuns, 0)
  if (mathRuns > 0) degradation.push({ kind: 'formula-dropped', count: mathRuns })
  if (truncated !== undefined) degradation.push(truncated)
