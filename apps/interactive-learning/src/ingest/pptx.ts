...
import { listZipEntries, readZipEntry, readZipText, ZipFormatError, type ZipEntry } from './zip.ts'
import {
  quoteHashOf,
  type ParsedBlock,
  type ParsedSource,
...
function shapeParagraphs(xml: string): readonly string[] {
 ...
 */
function slideOrder(bytes: Uint8Array): readonly string[] {
  ...
  if (presentation !== undefined && rels !== undefined) {
    ...
    for (const match of presentation.matchAll(SLIDE_ID)) {
      ...
      if (name !== undefined && available.has(name)) ordered.push(name)
    }
  ...
  return [...available].sort((left, right) =>
    Number.parseInt(/(\d+)/.exec(left)?.[1] ?? '0', 10)
...
function notesFor(
  bytes: Uint8Array,
  parts: ReadonlyMap<string, ZipEntry>,
  slidePath: string,
): string {
 ...
 */
export function parsePptxSource(
  bytes: Uint8Array,
  ...
  let slides: readonly string[] = []
  let parts: ReadonlyMap<string, ZipEntry> = new Map()
  try {
