...
import { parseDocxSource } from './docx.ts'
import { parsePdfSource } from './pdf.ts'
import { parsePptxSource } from './pptx.ts'
import {
  CODE_EXTENSIONS,
  MARKDOWN_EXTENSIONS,
...
import { slugify, type ParsedSource } from './types.ts'
...
export async function parseSource(
  bytes: Uint8Array,
  fileName: string,
  sourceId = slugify(titleOf(fileName)),
): Promise<ParsedSource> {
  const extension = extensionOf(fileName)
  const title = titleOf(fileName)
  if (extension === 'pdf') return await parsePdfSource(bytes, { sourceId, title })
  if (extension === 'docx') return parseDocxSource(bytes, { sourceId, title })
  if (extension === 'pptx') return parsePptxSource(bytes, { sourceId, title })
  if (
