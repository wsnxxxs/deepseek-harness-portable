...
const ACTIVITY_KINDS = ['parameter_explorer', 'process_stepper', 'structure_compare'] as const
const MATH_BINARY = new Set(['add', 'sub', 'mul', 'div', 'pow', 'min', 'max'])
const MATH_UNARY = new Set(['abs', 'neg', 'exp', 'log', 'sqrt', 'sigmoid', 'tanh', 'relu', 'leaky_relu', 'step', 'normpdf'])
...
function jsonBytes(value: unknown): number | undefined {
  try { return new TextEncoder().encode(JSON.stringify(value)).byteLength } catch { return undefined }
}

function validateJson(value: unknown, path: string, issues: string[], depth = 0): void {
  if (depth > 12) {
...
function validateMath(value: unknown, parameterIds: ReadonlySet<string>, path: string, issues: string[], depth = 1): void {
  if (depth > 8) {
  ...
  if (value.op === 'variable') {
    ...
    if (typeof value.name !== 'string' || (!parameterIds.has(value.name) && value.name !== 'x')) {
      issues.push(`${path}.name must be x or a declared parameter id`)
    }
    return
  }
  if (MATH_BINARY.has(value.op)) {
    onlyKeys(value, ['op', 'left', 'right'], path, issues)
...
function validateV1Payload(kind: unknown, payload: unknown, issues: string[]): void {
  if (!record(payload)) {
  ...
  if (kind === 'parameter_explorer') {
    ...
    for (const side of ['left', 'right'] as const) {
      ...
      if (!Array.isArray(value.items) || value.items.length < 1 || value.items.length > 20) {
        issues.push(`activity.payload.${side}.items must contain 1 to 20 items`)
      } else {
...
export function parseLearningActivity(value: unknown): LearningActivityV1 {
  ...
  if (!(ACTIVITY_KINDS as readonly unknown[]).includes(value.kind)) issues.push('activity.kind is unknown')
...
function validateFocus(value: unknown, issues: string[]): void {
  if (!record(value)) {
  ...
  if (value.progress !== undefined) {
    if (!record(value.progress)) issues.push('activity.focus.progress must be an object')
      ...
      if (value.progress.total !== undefined) integer(value.progress.total, 'activity.focus.progress.total', issues, 1)
    }
  }
}
...
function validateInput(value: unknown, issues: string[]): void {
  ...
  if (value.kind === 'single_choice') {
    ...
    onlyKeys(value, ['kind', 'placeholder', 'maxLength'], 'activity.input', issues)
    if (value.placeholder !== undefined) text(value.placeholder, 'activity.input.placeholder', issues, 500)
    if (value.maxLength !== undefined) integer(value.maxLength, 'activity.input.maxLength', issues, 1)
  } else if (value.kind === 'number') {
    onlyKeys(value, ['kind', 'min', 'max', 'step'], 'activity.input', issues)
...
function validateFrame(value: unknown, path: string, issues: string[]): void {
  if (!record(value)) {
...
function validateVisual(value: unknown, phase: 'question' | 'reveal', issues: string[]): void {
  ...
  if (value.kind === 'process') {
    ...
    const required = value.kind === 'parameter' ? ['parameters', 'xAxis', 'curves'] : ['left', 'right', 'alignments']
...
export function parseLearningActivityV2(value: unknown): LearningActivityV2 {
  ...
  if (value.phase === 'question') {
    ...
    if (value.visual !== undefined) validateVisual(value.visual, 'question', issues)
    text(value.fallbackMarkdown, 'activity.fallbackMarkdown', issues, 16_000)
    ...
    if (value.visual !== undefined) validateVisual(value.visual, 'reveal', issues)
    if (!record(value.animation)) issues.push('activity.animation must be an object')
      ...
      if (value.animation.preferredDurationMs !== undefined) integer(value.animation.preferredDurationMs, 'activity.animation.preferredDurationMs', issues)
