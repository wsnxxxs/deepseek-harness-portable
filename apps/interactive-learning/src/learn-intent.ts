...
export function classifyLearnIntent(input: string): LearnIntentDecision {
  ...
  if (RESOURCE_SOFTWARE_TASK.test(text)) {
    return decision('not-learn', 'coding-task', 'software implementation task shaped like a study artifact')
  }
  if (RESOURCE_CREATION.test(text) && !NEGATED_RESOURCE_CREATION.test(text) && !RESOURCE_SOFTWARE_TASK.test(text)) {
    return decision('learn', 'resource-creation', 'the learner asks for a study artifact')
  }
  ...
  if (CALCULATION_TASK.test(text) && !EXPLICIT_LEARNING.test(text)) {
    return decision('not-learn', 'calculation-task', 'calculation or problem-solving task')
  }
  if (PERSONAL_TROUBLESHOOTING.test(text)) {
    return decision('not-learn', 'troubleshooting-task', 'personal troubleshooting request')
  }
  ...
  if (CURRENT_FACT_LOOKUP.test(text)) {
  ...
  if (FACTUAL_LOOKUP.test(text)) {
    return decision('not-learn', 'factual-lookup', 'request for a stable factual value')
  }
