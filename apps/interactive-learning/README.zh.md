...
- `study_map`：带章节/页码锚点、先修关系和概念角色的参考材料导览；也可显示学习库中已确认概念卡的掌握度、到期复习和失效锚点；
...
独立 fresh transfer 完成后，模型可以通过 Host 提议保存一张 `concepts/*.md` 概念卡；
只有学习者明确确认才会写入。卡片的 `due` 会在会话开始时作为可选复习提示注入，
不会阻塞当前问题。
