...
import type {
  TeamMemberView, TeamTaskMutationResult, TeamTaskView, TeamView,
...
/** A cleared form. */
export const EMPTY_DRAFT: TaskDraft = { subject: '', description: '', blockers: '', scopes: '' }
 ...
 */
export function memberStatusKey(status: TeamMemberView['status']): ClusterKey {
  switch (status) {
 ...
 */
export function taskStatusKey(status: TeamTaskView['status']): ClusterKey {
  switch (status) {
 ...
 */
export function isTeamWorthShowing(view: TeamView): boolean {
  return view.members.length > 1 || view.tasks.length > 0
}

/** Whether a teammate row can be opened as its own conversation. */
export function canOpenMember(member: TeamMemberView): boolean {
...
): readonly { readonly member: TeamMemberView; readonly depth: number }[] {
...
export type MutationCarrier =
  ...
  | { readonly ok: true; readonly value: TeamTaskMutationResult }
 ...
 */
export function mutationError(result: MutationCarrier): string | undefined {
...
export function rootSessionId(
  ...
  // A malformed lineage (a cycle written by a buggy host) must not hang the
  // browser: the visited set turns it into a bounded walk that stops on the
