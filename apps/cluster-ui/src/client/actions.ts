...
// Type-only imports pull the declaration merges (ctx.sessions, ctx.remote and
// the agentTeams namespace) into this compilation unit without adding a
// runtime module edge.
...
import { canOpenMember, rootSessionId } from './model.ts'
 ...
 */
export function createClusterActions(ctx: ClientContext): ClusterActions {
  const sessions = ctx.sessions
  const agentTeams = ctx.remote.agentTeams

  /** The session that owns the Team the given session belongs to. */
  const leadOf = (sessionId: SessionId): SessionId => rootSessionId(sessionId, (id) => {
    const address = sessions.binding(id as SessionId)?.session.getSnapshot().subagent?.address
