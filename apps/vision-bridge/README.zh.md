...
4. Agent 需要检查本地截图、图表、界面或讲义页面时，会调用 `view_image`。本地图片或 PDF 使用 `path`（绝对路径或相对当前会话工作区的路径）；PDF 可用 1-based 的 `page` 指定页面，默认第 1 页。工具会在本机临时把 PDF 页渲染为 PNG，用完即删；重新分析当前会话历史中已有的图片则使用 `attachmentId`，两者不能同时提供；`prompt` 可指定要提取或分析的内容。
